...
class LoginHandler(BlogHandler):
    ...
    def render_login_form(self, usererror="", passerror=""):
        """ Render the login form with or without an error, based on parameters """
        t = jinja_env.get_template("login.html")
        response = t.render(passerror=passerror, usererror=usererror)
        self.response.out.write(response)
    ...
    def post(self):
        ...
        if not user:
            self.render_login_form(usererror="Invalid username")
        elif hashutils.valid_pw(submitted_username, submitted_password, user.pw_hash):
            self.login_user(user)
            self.redirect('/blog/newpost')
        else:
            self.render_login_form(passerror="Invalid password")
